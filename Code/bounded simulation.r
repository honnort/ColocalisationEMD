...
bounded.simulation = function(shared.proportion,output.folder.name,number.of.simulations,number.of.time.points,image.width,image.height){
...
if (shared.objects>0){
	...
	for (i in 1:shared.objects){
		...
		for (j in 2:number.of.time.points){
			...
			holding.location <- c(-1,-1)
			
			while (holding.location[1]<0|holding.location[1]>(image.width-1)|holding.location[2]<0|holding.location[2]>(image.height-1)){
		
				holding.angle <- runif(1)*2*pi
				
				holding.location[1] <- centre.locations[1,i,(j-1),1]+movement.speed*cos(holding.angle)
				holding.location[2] <- centre.locations[1,i,(j-1),2]+movement.speed*sin(holding.angle)
				
			}
			
			centre.locations[,i,j,1] <- holding.location[1]
			centre.locations[,i,j,2] <- holding.location[2]
	...
	for (j in 1:number.of.simulations){
		...
		for (k in 2:number.of.time.points){
			...
			while (holding.location[1]<0|holding.location[1]>(image.width-1)|holding.location[2]<0|holding.location[2]>(image.height-1)){
				...
				holding.location[1] <- centre.locations[j,i,(k-1),1]+movement.speed*cos(holding.angle)
				holding.location[2] <- centre.locations[j,i,(k-1),2]+movement.speed*sin(holding.angle)
				
			}
			
			centre.locations[j,i,k,1] <- holding.location[1]
			centre.locations[j,i,k,2] <- holding.location[2]
